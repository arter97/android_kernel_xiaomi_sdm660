...
static bool check_init_files(void)
{
	int i;
	const char* files[] = {
		WLAN_MAC_FILE,
	...
	for (i = 0; i < ARRAY_SIZE(files); i++) {
		if (!firmware_exists(files[i])) {
			pr_err("qcacld: %s not present\n", files[i]);
			return false;
		}
	}
 ...
 */
static void __hdd_module_init(struct work_struct *work)
{
	int ret = 0;
	...
	while (!check_init_files()) {
		pr_err("qcacld: still waiting for firmwares to appear\n");
		msleep(500);
	}
...
{
	INIT_DELAYED_WORK(&init_work, __hdd_module_init);
	queue_delayed_work(system_freezable_power_efficient_wq, &init_work, 0);
